...
namespace AlgoDemos.codecamp.arrays
{
    ...
    public class buysellstockfour
    {
        ...
        public int MaxProfit(int k, int[] prices)
        {
            ...
            return MaxProfitsDP(profitsArray, profitsArray.Length, k);
        ...
        /// <summary>
        /// times out on leetcode at k=100 - 208 / 210 testcases passed
        /// </summary>
        /// <param name="profitsArray"></param>
        /// <param name="n"></param>
        /// <param name="k"></param>
        /// <returns></returns>
        int MaxProfitsDP(int[] profitsArray, int n, int k)
        {
            int[,] dp = new int[n, k];
            ...
            for (int x = 1; x < n; x++)
            {
                ...
                dp[x, 0] = FindMaxSubArraySum(profitsArray, 0, x-1);
            }

            for (int y = 0; y < k; y++)
            {
                // only one profit transaction left
                dp[0,y] = System.Math.Max(profitsArray[0], 0);
            }

            dp[0,0] = System.Math.Max(profitsArray[0], 0);
            ...
            for (int y = 1; y < k; y++)
            {
                for (int x = 1; x < n; x++)
                {
                    ...
                    for (int z = 0; z < x; z++)
                    {
                        res = System.Math.Max(res, dp[z, y - 1] + FindMaxSubArraySum(profitsArray, z+1, x));
                    }
                    dp[x, y] = res;
                }
            }
        ...
        Dictionary<string, int> _cacheMaxSubArraySum = new Dictionary<string, int>();
        private int FindMaxSubArraySum(int[] profitsArray, int start, int end)
        {
            ...
            if (_cacheMaxSubArraySum.ContainsKey(key))
            {
                return _cacheMaxSubArraySum[key];
            }
